...
class InvoiceController {
    ...
    public function store() {
        ...
        try {
            ...
            $pat = $this->patientModel->findById($invoiceData['patient_id']);
            if ($pat) {
                $notif->create($pat['user_id'], 'Hóa đơn mới chờ thanh toán', 'Bạn có hóa đơn mới #' . $invoiceId . ' trị giá ' . number_format($invoiceData['patient_payment'] > 0 ? $invoiceData['patient_payment'] : $invoiceData['final_amount'], 0, ',', '.') . ' VNĐ cần thanh toán.');
            }
    ...
    public function markPaid() {
        ...
        try {
            ...
            $invoice = $this->invoiceModel->findById($id);
            if ($invoice) {
                $pat = $this->patientModel->findById($invoice['patient_id']);
                if ($pat) {
                    $notif->create($pat['user_id'], 'Thanh toán thành công', 'Hóa đơn #' . $id . ' đã được thanh toán thành công qua hình thức ' . strtoupper($method) . '.');
                }
            }
    ...
    public function vnpayReturn() {
        ...
        if ($isValid) {
            ...
            if ($responseCode === '00') {
                ...
                $pat = $this->patientModel->findById($invoice['patient_id']);
                if ($pat) {
                    $notif->create($pat['user_id'], 'Thanh toán thành công', 'Hóa đơn #' . $invoiceId . ' đã được thanh toán thành công qua VNPay.');
                }
