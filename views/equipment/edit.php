    ...
    <form method="POST" action="index.php?page=equipment&action=update">
        ...
        <div class="row">
            ...
            <div class="col-md-6 mb-3">
                ...
                <input type="number" class="form-control" id="quantity" name="quantity" required min="0" value="<?= $equipment['quantity'] ?>">
            </div>
        </div>
        <div class="mb-3">
            <label for="status" class="form-label">Trạng thái</label>
            <select class="form-select" id="status" name="status">
                <option value="available" <?= $equipment['status'] == 'available' ? 'selected' : '' ?>>Sẵn sàng sử dụng</option>
                <option value="maintenance" <?= $equipment['status'] == 'maintenance' ? 'selected' : '' ?>>Đang bảo trì / Hư hỏng</option>
            </select>
        </div>
        <div class="mb-4">
            <label for="description" class="form-label">Mô tả / Ghi chú</label>
            <textarea class="form-control" id="description" name="description" rows="4"><?= htmlspecialchars($equipment['description'] ?? '') ?></textarea>
        </div>
